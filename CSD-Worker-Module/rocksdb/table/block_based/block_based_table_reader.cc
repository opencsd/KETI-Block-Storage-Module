...
Status ReadBlockFromFileByPBA(const UncompressionDict& uncompression_dict,
  std::vector<uint64_t> block_size_, std::string dev_name_, std::vector<uint64_t> block_offset_, 
  ...
  PbaBlockFetcher pba_block_fetcher(
    ...
    options_, &contents, block_type_, using_zstd, uncompression_dict);
...
Status BlockBasedTable::Open(
  ...
  // printf("called block_based_table_reader.cc/BlockBasedTable::Open\n");
  table_reader->reset();
...
Status BlockBasedTable::BlockOpen(
      ...
      std::vector<uint64_t> _block_offset, std::vector<uint64_t> _block_size, int _block_id, std::string _dev_name,
...
Status BlockBasedTable::RetrieveBlockByOffset(const int fd,
    ...
    GetContext* get_context, BlockCacheLookupContext* lookup_context,
    bool for_compaction, bool use_cache, bool wait_for_cache) const {
  // std::cout << "[func call] block_based_table.cc > BlockBasedTable::RetrieveBlockByOffset" << std::endl;
  assert(block_entry);
  assert(block_entry->IsEmpty());
...
Status BlockBasedTable::RetrieveBlockByPBA(
  ...
  std::unique_ptr<Block> block;
  {
    s = ReadBlockFromFileByPBA(uncompression_dict,
  ...
  //  printf("\n-----------------------\n"); 
  // std::cout << "####block read size: " << block_entry->GetValue()->size() << std::endl;
  // for(size_t j = 0; j < block_entry->GetValue()->size(); j++) {
  //   printf("%02X ",(u_char)block_entry->GetValue()->data()[j]);
  // }
  // printf("\n------------------------\n\n"); 
