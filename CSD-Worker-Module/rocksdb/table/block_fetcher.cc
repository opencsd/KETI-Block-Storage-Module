...
IOStatus PbaBlockFetcher::ReadBlockContentsByPBA() {
  ...
  for(size_t i=0; i<block_offset.size(); i++){
    // std::cout << "#dev_name: " << dev_name.c_str() <<"| handle_offset : "<< block_offset[i] << "| block_size: " << block_size[i] << std::endl;
    lseek(dev_fd,block_offset[i],SEEK_SET);
    size_t read_size = read(dev_fd, internal_buf+total_read_size, block_size[i]);
    if(read_size != block_size[i]){
      std::cout << "[block_fetcher.cc 516] read_size != block_size" << std::endl;
    }
  ...
  // slice 만들땐 원래 블록 사이즈에서 footer_block_trailer_size를 꼭 빼야 valid임
  *contents_ = BlockContents(Slice(slice_.data(), block_size_with_trailer - footer_block_trailer_size));

    // printf("\n-------------------------------------------\n"); 
    // std::cout << "#block_buf : " << std::endl;
    // for(size_t j = 0; j < total_read_size; j++) {
    //   printf("%02X ",(u_char)internal_buf[j]);
    // }
    // printf("\n-------------------------------------------\n\n"); 
...
//     }
  ...
  // printf("\n-----------------------\n"); 
  // std::cout << "#block read size: " << read_size << "| block_size_with_trailer: " << block_size_with_trailer << "| contents_size: " << contents_->data.size() << std::endl;
  // for(size_t j = 0; j < contents_->data.size(); j++) {
  //   printf("%02X ",(u_char)contents_->data[j]);
  // }
  // printf("\n------------------------\n\n"); 
...
//     PERF_TIMER_GUARD(block_decompress_time);
//     // compressed page, uncompress, update cache
//     std::cout << "#contents : " << contents_->data.size() << std::endl;
//     UncompressionContext context(kZSTD);
//     UncompressionInfo info(context, uncompression_dict_, kZSTD);
//     io_status_ = status_to_io_status(UncompressBlockContentsSimple(
//         info, slice_.data(), block_size, contents_, 4, memory_allocator_));
    
//     // printf("\n----------------------------------------------------------------------------------------\n"); 
//     std::cout << "#uncompress contents : " << contents_->data.size() << std::endl;
//     // for(size_t j = 0; j < contents_->data.size(); j++) {
//     //   printf("%02X ",(u_char)contents_->data[j]);
//     // }
//     // printf("\n----------------------------------------------------------------------------------------\n\n"); 
