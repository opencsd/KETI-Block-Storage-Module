...
class Slice {
 public:
  ...
  std::string keti_get_sliced_string(int offset, int length) const {
    return std::string(data_+offset, length);
  }

  int keti_get_table_index_number() const{
    std::string sliced_char = std::string(data_, 4);
    char recersed_char[4];
    recersed_char[0] = sliced_char[3];
    recersed_char[1] = sliced_char[2];
    recersed_char[2] = sliced_char[1];
    recersed_char[3] = sliced_char[0];
    int table_index_number = *((int32_t *)recersed_char);
    
    return table_index_number;
  }
